...
(function () {
  ...
  function sameForm(a, b) { return a.closest('form') === b.closest('form'); }
  function move(node, target) {
    if (node && node.parentNode !== target && sameForm(node, target)) target.appendChild(node);
  }
  function setup() {
    ...
    info.appendChild(el('h2', 'crp-card-title', 'Informações da solicitação'));
    ...
    actions.appendChild(el('h2', 'crp-action-title', 'Suas ações nesta etapa'));
    var content = el('div', 'crp-action-content');
    actions.appendChild(content);
    right.appendChild(actions);
    original.forEach(function (node) { content.appendChild(node); });

    state = { root: root, body: body, main: main, left: left, right: right,
      info: info, tabs: tabs, summary: summary, actions: actions,
      content: content, items: [], active: null };
    addTab(summary, 'Resumo');
    return state;
  }

  function addTab(panel, label, count) {
    if (!panel || state.items.some(function (i) { return i.panel === panel; })) return;
    ...
    var item = { panel: panel, button: button, label: label, count: count };
    ...
    button.addEventListener('keydown', function (e) {
      ...
      else if (e.key === 'End') index = state.items.length - 1;
      else index = (index + (e.key === 'ArrowRight' ? 1 : -1) + state.items.length) % state.items.length;
      activate(state.items[index]);
      state.items[index].button.focus();
    });
  }
  function activate(item) { state.active = item; render(); }
  function render() {
    state.items.forEach(function (item) {
      ...
      var count = item.count && state.root.querySelector('#commands .' + item.count);
      var text = item.label + (count ? ' · ' + count.textContent.trim() : '');
  ...
  function update() {
    ...
    Array.prototype.slice.call(s.body.childNodes).forEach(function (node) {
      if (node !== s.left && node !== s.right) s.content.appendChild(node);
    });
    ...
    if (s.items.indexOf(s.active) < 0) s.active = s.items[0];

    // Somente grupos simples de consulta vão para a esquerda.
    // Os elementos originais continuam dentro de BoxFrmExecute.
    s.body.querySelectorAll('table.form').forEach(function (table) {
      var wrapper = table.closest('table[id="FrmExecute"]') || table;
      var interactive = wrapper.querySelector('input:not([type="hidden"]), select, textarea, button, [contenteditable="true"]');
    ...
    if (instructions && sameForm(instructions, s.right)) {
      ...
      if (instructions.parentNode !== instructionsCard) {
        ...
        // Move o elemento original, preservando textos e formatação.
        instructionsCard.appendChild(instructions);
    
        // Remove apenas o recipiente antigo se estiver vazio.
        if (
          oldContainer &&
    ...
    var checklist = s.root.querySelector('#ContainerConclusionCheckList');
    if (checklist && sameForm(checklist, s.right)) {
      checklist.classList.add('crp-card', 'crp-checklist');
      if (!checklist.querySelector('h5, h2')) checklist.insertBefore(el('h2', 'crp-card-title', 'Checklist'), checklist.firstChild);
      if (checklist.parentNode !== s.right) s.right.insertBefore(checklist, s.actions);
    }
    [
      ...
      if (!panel || s.items.some(function (item) { return item.panel === panel; })) return;
      if (panel.hidden || getComputedStyle(panel).display === 'none') return;
      addTab(panel, def[1], def[2]);
    });

    // Mantém todos os atalhos e ações auxiliares acessíveis.
    ...
    if (commands && !commands.dataset.crpBound) {
      ...
      commands.addEventListener('click', function (e) {
        ...
        if (!link) return;
        var item = s.items.find(function (i) { return link.getAttribute('href') === '#' + i.panel.id; });
        if (!item) return;
        e.preventDefault();
        activate(item);
    ...
    // IDs confirmados no HTML enviado; não altera ações nem validação.
    ['BtnSend', 'btnFinish'].forEach(function (id) {
      var button = s.root.querySelector('#' + id);
      if (button) button.classList.add('crp-action-primary');
    });
    var review = document.getElementById('customBtn_Revisão solicitada');
    if (review && s.root.contains(review)) {
      review.classList.add('crp-action-review');
      // O Zeev forneceu essas cores inline com !important.
  ...
  function refresh() {
    ...
    try { update(); }
      ...
      if (observer) observer.observe(document.body, {
        ...
        attributes: true, attributeFilter: ['hidden', 'style', 'class', 'disabled', 'readonly']
      });
    }
  }
  function schedule() { clearTimeout(timer); timer = setTimeout(refresh, 100); }
  window.CRPForm = { baseVersion: '2.0.0', atualizar: refresh };
  function start() {
...
(function () {
  ...
  function ajustarSalvar() {
    ...
    // Mantém o controle no mesmo formulário HTML.
    ...
    root.querySelectorAll('table[mult="S"]').forEach(function (table) {
      ...
      Array.prototype.forEach.call(table.rows, function (row) {
        ...
        cell.querySelectorAll(
          ...
          // Altera somente o texto apresentado.
